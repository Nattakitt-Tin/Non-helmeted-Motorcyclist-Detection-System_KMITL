...
def main(video_name_list, cursor, bike_h, max_speed_ratio, show=True):
    for video_name in video_name_list:
        ...
        while True:
                ...
                if h > bike_h:
                    ...
                    for p in p_list:
                        ...
                        if -accepted_diff < xc-p.x < accepted_diff and -accepted_diff < yc-p.y < accepted_diff: # center around some object in previous frame, check if its same object
                            p.update(xc, yc, frame_copy, x, y, w, h, bottom)
                            exist = True
                            break
                    if not exist:
            ...
            if show:
                cv2.imshow('BGR', cv2.resize(disp_frame, (int(frame_copy.shape[1]*0.7),int(frame_copy.shape[0]*0.7))))
                cv2.imshow('Binary', cv2.resize(binary, (int(frame_copy.shape[1]*0.7),int(frame_copy.shape[0]*0.7))))
...
'''
to be replace from ui
cpt_range = 50 #distance from bottom we accept to capture (to get the biggest obj)
disc_ratio = 0.3 #Interest area, from 0. to 1. 
bike_h = 200 #expected bike height/width, in pixel
max_speed_ratio = 0.2 # ratio on frame height that we accept when 2 object between frame move
accepted_bike_threshold = 0.1 #accuracy accept
img_num = 6170 #started img number
'''
cursor = [(0,200),(1000,800)]

main(video_name_list=['a1'], cursor=cursor, bike_h=200, max_speed_ratio=0.2)